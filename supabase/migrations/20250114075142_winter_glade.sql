/*
  # Update manga and daily updates images

  1. Changes
    - Updates both manga cover_url and daily_updates image_url with consistent image URLs
*/

-- Update manga cover URLs
UPDATE manga
SET cover_url = CASE 
...
-- Update daily updates image URLs to match manga cover URLs
UPDATE daily_updates
SET image_url = (
  SELECT cover_url 
  FROM manga 
  WHERE manga.id = daily_updates.manga_id
);